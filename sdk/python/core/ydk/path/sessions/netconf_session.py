    ...
    def __init__(self,
        ...
        if repo is None:
            if len(public_key_path) == 0:
                ...
                super().__init__(address, username,
                                 private_key_path, public_key_path,
                                 port, protocol, on_demand,
                                 common_cache, timeout)
        else:
            if len(public_key_path) == 0:
                super().__init__(repo, address, username, password,
                                 port, protocol,
                                 on_demand, timeout)
            else:
                super().__init__(repo, address, username,
                                 private_key_path, public_key_path,
                                 port, protocol, on_demand,
                                 common_cache, timeout)

    def get_root_schema(self):
        return super().get_root_schema()

    def invoke(self, rpc):
        return super().invoke(rpc)

    def execute_netconf_operation(self, rpc):
        return super().execute_netconf_operation(rpc)
